...
function ComboBoxWindowProc(Window: HWND; Msg: UINT; wParam: Windows.WPARAM; lparam: Windows.LPARAM;
  uISubClass: UINT_PTR; dwRefData: DWORD_PTR): LRESULT; stdcall;
var
  DC: HDC;
...
begin
  ...
  begin
    if iPartId in [MENU_BARBACKGROUND, MENU_POPUPITEM, MENU_POPUPGUTTER, MENU_POPUPSUBMENU, MENU_POPUPSEPARATOR,
      MENU_POPUPCHECK, MENU_POPUPCHECKBACKGROUND] then
    begin
      LCanvas := TCanvas.Create;
      try
        LCanvas.Handle := hdc;
...
begin
  ...
  begin
    ...
    try
      ...
      Result := DefaultQuestionDialog(UTF8Encode(unicodestring(pszWindowTitle)), UTF8Encode(unicodestring(pszContent)),
        DlgType, Buttons, 0);
...
begin
  ...
  try
    ...
    begin
      ...
      if iPartId in [TABP_TABITEMLEFTEDGE, TABP_TABITEMBOTHEDGE, TABP_TOPTABITEMLEFTEDGE, TABP_TOPTABITEMBOTHEDGE] then
      begin
        // Draw left border
...
procedure CloseAllDarkThemes;
var
  hTheme: THANDLE;
begin
  if Assigned(ThemeClass) then
  begin
    for hTheme in ThemeClass.Keys do
